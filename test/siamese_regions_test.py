...
def main(dataset_full, model, weights, device, feature_dim, regions_k):
...
if __name__ == '__main__':
    ...
    try:
        ...
        usage()
        sys.exit(2)
    dataset_full, model, weights, device, feature_dim, regions_k = None, None, None, None, None, None
    for opt, arg in opts:
        if opt in ('--help'):
            usage()
            sys.exit()
    ...
    with torch.cuda.device(device):
        try:
            main(dataset_full, model, weights, device, feature_dim, regions_k)
        except:
